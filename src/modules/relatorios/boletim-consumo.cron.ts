...
/**
 * Cron do Relatório de Gestão de Energia (CONSUMO). MESMA lógica do de geração
 * ([[boletim-semanal.cron]]): roda a cada minuto, dispara UMA vez no dia_semana + horário
 * (SP), guardado por `ultimo_envio`. Só envia com `ativo=true` (nasce false → no-op).
 */
@Injectable()
export class BoletimConsumoCron {
  private readonly logger = new Logger(BoletimConsumoCron.name);

  constructor(private readonly envio: BoletimConsumoEnvioService) {}
  ...
  async tick(): Promise<void> {
    ...
    this.logger.log(`[boletim-consumo-cron] ${hoje} ${hhmm} (dia ISO ${isodow}) — disparando envio real`);
    try {
      ...
      this.logger.log(`[boletim-consumo-cron] concluído: ok=${ok} erro=${err} total=${r.alvos.length}`);
    } catch (e: any) {
      this.logger.error(`[boletim-consumo-cron] fatal: ${e?.stack || e?.message}`);
    }
  }
